...
describe("Output Condition Routes", () => {
  describe("GET", () => {
    ...
    it("should reject invalid list path parameters through remaining handler/domain validation", async () => {
      const response = await request(server)
        .get("/api/v2/automations/not-a-number/conditions/output")
        .expect(400);
    ...
    it("should reject invalid item path parameters through remaining handler/domain validation", async () => {
      const response = await request(server)
        .get("/api/v2/automations/1/conditions/output/not-a-number")
        .expect(400);
  ...
  describe("POST", () => {
    ...
    it("should reject missing required fields through contract middleware", async () => {
      ...
      assertContractBadRequest(response, "/api/v2/automations/1/conditions/output", invalidBody);
    });
    ...
    it("should reject invalid path parameters through remaining handler/domain validation", async () => {
      const response = await request(server)
        .post("/api/v2/automations/not-a-number/conditions/output")
        .send({
          groupType: "oneOf",
          operator: "greater",
          comparisonValue: 20,
  ...
  describe("PATCH", () => {
    it("should return 200", async () => {
      assert.equal((await app.get("sprootDB").getOutputConditionsAsync(1))[2].comparisonValue, 20);
      const response = await request(server)
      ...
      validateMiddlewareValues(response);
      assert.equal((await app.get("sprootDB").getOutputConditionsAsync(1))[2].comparisonValue, 30);
      assert.deepInclude(response.body["content"]["data"], {
        id: 3,
        comparisonValue: 30,
      });
    ...
    it("should reject invalid path parameters through remaining handler/domain validation", async () => {
      const response = await request(server)
        .patch("/api/v2/automations/not-a-number/conditions/output/not-a-number")
  ...
  describe("DELETE", () => {
    it("should return 200", async () => {
      assert.lengthOf(await app.get("sprootDB").getOutputConditionsAsync(1), 3);
      const response = await request(server)
        .delete("/api/v2/automations/1/conditions/output/3")
        .expect(200);
    ...
    it("should reject invalid path parameters through remaining handler/domain validation", async () => {
      const response = await request(server)
        .delete("/api/v2/automations/not-a-number/conditions/output/not-a-number")
        .expect(400);
