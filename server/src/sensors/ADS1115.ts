...
export class ADS115 extends SensorBase {
  ...
  protected async getReadingFromDeviceAsync(): Promise<number> {
      ...
      const mux = `${this.pin}+'GND'` as '0+1' | '0+3' | '1+3' | '2+3' | '0+GND' | '1+GND' | '2+GND' | '3+GND';
...
class Ads1115Device {
  ...
  public static MUX: Record<'0+1' | '0+3' | '1+3' | '2+3' | '0+GND' | '1+GND' | '2+GND' | '3+GND', number> = {
  ...
  get gain(): Number { return this.#gain };
  ...
  async readReg16Async(register: number) {
    ...
    if (buff.length !== 2) {
      throw new Error(`Invalid read length from register 0x${register.toString(16)}: ${buff.length}`)
    }
    return (buff[0]! << 8) | buff[1]!
  }

  async readResultsAsync() { return (await this.readReg16Async(0x00)) >> this.#shift }
  ...
  async measureAsync(mux: '0+1' | '0+3' | '1+3' | '2+3' | '0+GND' | '1+GND' | '2+GND' | '3+GND', gain: '2/3' | '1' | '2' | '4' | '8' | '16' = '2/3') {
    ...
    await this.writeConfigAsync(config | calculatedGain | muxValue | Ads1115Device.START_CONVERSION)
