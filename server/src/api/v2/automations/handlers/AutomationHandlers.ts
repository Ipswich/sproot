...
export async function getAsync(request: Request, response: Response) {
  ...
  try {
    const automations = await sprootDB.automations.getAutomationsAsync();
    automationResponse = {
      statusCode: 200,
      content: {
        data: automations,
...
export async function getByIdAsync(request: Request, response: Response) {
  ...
  try {
    const automation = (
      await sprootDB.automations.getAutomationAsync(parseInt(request.params["automationId"]))
    )[0];
    if (automation == null) {
      return {
        statusCode: 404,
...
export async function deleteAsync(request: Request, response: Response) {
  ...
  try {
    const automation = await sprootDB.automations.getAutomationAsync(
      parseInt(request.params["automationId"]),
    );
