...
export async function getAsync(
  ...
  try {
    if (
      ...
      const automations = await sprootDB.actions.output.getAsync(
    ...
    const actions = await sprootDB.actions.output.getAllAsync();
    automationResponse = {
      statusCode: 200,
      content: {
        data: actions,
...
export async function getByIdAsync(
  ...
  try {
    ...
    const automation = (await sprootDB.actions.output.getOutputActionAsync(outputActionId))[0];
    if (automation == null) {
      automationResponse = {
        statusCode: 404,
        error: {
...
export async function deleteAsync(
  ...
  try {
    const outputActionId = parseInt(request.params["outputActionId"] ?? "");
    const action = (await sprootDB.actions.output.getOutputActionAsync(outputActionId))[0];
    if (action == null) {
      automationResponse = {
        statusCode: 404,
        error: {
