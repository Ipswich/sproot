...
export function sensorChartDataHandler(request: Request, response: Response): SuccessResponse | ErrorResponse {
  ...
  if (String(request.query["latest"]).toLowerCase() == "true") {
    for(const readingType in chartData.data){
      chartData.data[readingType as ReadingType] = chartData.data[readingType as ReadingType].slice(-1);
    }
  }
