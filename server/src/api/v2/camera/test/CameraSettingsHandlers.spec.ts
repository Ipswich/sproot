...
describe('CameraSettingsHandlers', () => {
  ...
  describe('updateCameraSettingsAsync', () => {
    ...
    it('should return 200 for successfully updating camera settings', async () => {
      ...
      const result = await updateCameraSettingsAsync(mockRequest, mockResponse) as SuccessResponse;
    ...
    it('should return 400 for invalid timelapseStartTime format', async () => {
      ...
      assert.equal(result.statusCode, 400);
      assert.include(result.error.details, 'timelapseStartTime must be a string in HH:MM format, or null');
    });
    ...
    it('should return 400 for invalid timelapseEndTime format', async () => {
      ...
      assert.equal(result.statusCode, 400);
      assert.include(result.error.details, 'timelapseEndTime must be a string in HH:MM format, or null');
    });
    ...
    it('should return 400 for mismatched timelapseStartTime and timelapseEndTime', async () => {
      ...
      assert.equal(result.statusCode, 400);
      assert.include(result.error.details, 'Both timelapseStartTime and timelapseEndTime must be provided or both must be null');
