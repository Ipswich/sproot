...
describe("PCA9685.ts tests", function () {
  ...
  it("should update and apply states with respect to control mode", async function () {
    ...
    pca9685.setNewOutputState(
      "1",
      <PCA9685State>{ isOn: true, value: 100 },
    ...
    pca9685.setNewOutputState(
      "1",
      <PCA9685State>{ isOn: false, value: 0 },
      ControlMode.schedule,
    );
  ...
  it("should throw errors for invalid values", async function () {
    ...
    pca9685.setNewOutputState(
      "1",
      <PCA9685State>{ isOn: true, value: 50 },
      ControlMode.schedule,
    );
    assert.throws(
      () => pca9685.executeOutputState(),
    ...
    pca9685.setNewOutputState(
      "1",
      <PCA9685State>{ isOn: true, value: 101 },
      ControlMode.schedule,
    );
    assert.throws(
      () => pca9685.executeOutputState(),
    ...
    await expect(
      pca9685.initializeOrRegenerateAsync(),
    ).to.eventually.be.rejectedWith("Pin 0 is already in use or is invalid");
  });
});
