...
export default function Journals() {
  ...
  const [tagsModalOpened, { open: openTagsModal, close: closeTagsModal }] = useDisclosure(false);
  const [newJournalModalOpened, { open: openNewJournal, close: closeNewJournal }] = useDisclosure(false);
  ...
  type JournalRowWithKey = JournalRow & { key: string; leaving?: boolean; appearing?: boolean };
  const [visibleRows, setVisibleRows] = useState<JournalRowWithKey[]>([]);

  // get "all" tags from journals
  const tagMap = new Map<number, { id: number; name?: string | null | undefined; color?: string | null | undefined }>();
  (getJournalsQuery.data ?? []).forEach((j) => {
    (j.tags ?? []).forEach((t) => {
      const tt = t as unknown as { id?: number; name?: string | null; color?: string | null };
      if (tt && typeof tt.id === 'number') tagMap.set(tt.id, { id: tt.id, name: tt.name, color: tt.color });
    });
  });
  ...
  useEffect(() => {
    if ((getJournalsQuery.data ?? []).length > 0 && filters.length === 0) {
      if (localStorage.getItem(journalsFiltersKey())) {
        setFilters(JSON.parse(localStorage.getItem(journalsFiltersKey()) || "[]"));
      } else {
  ...
  const filtered = (getJournalsQuery.data ?? []).filter((j) => {
    ...
    const journalTagIds = (j.tags ?? []).map((t) => {
    ...
    const matchesTag = selectedTagIds.length > 0 ? journalTagIds.some((id) => selectedTagIds.includes(id)) : false;
  ...
  const cmp = (a: JournalRow, b: JournalRow) => {
    ...
    const getVal = (jObj: JournalRow["journal"]) => {
      ...
      if (sortBy === "editedAt") return jObj.editedAt ? new Date(jObj.editedAt).getTime() : null;
      if (sortBy === "archivedAt") return jObj.archivedAt ? new Date(jObj.archivedAt).getTime() : null;
      ...
      sorted.forEach((s) => {
        ...
        if (existing) {
          next.push({ ...existing, leaving: false });
                  ...
                  next.push({ key: k, journal: s.journal, tags: s.tags ?? null, appearing: true });
        }
      });
  ...
  useEffect(() => {
    if (visibleRows.length === 0 && sorted.length > 0) {
      setVisibleRows(sorted.map((s) => ({ key: String(s.journal.id), journal: s.journal, tags: s.tags ?? null })));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
        ...
        <Stack>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            <div style={{ flex: 1, minWidth: 280, display: "flex", alignItems: "center", gap: 12 }}>
              <TagsPillsCombo allTags={[{ id: -1, name: "Archived", color: "#868e96" }, ...allTags]} value={filters} onChange={(newFilters) => {
                setFilters(newFilters);
                localStorage.setItem(journalsFiltersKey(), JSON.stringify(newFilters));
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <Menu withinPortal={false} position="bottom-end">
                <Menu.Target>
                  <Button variant="outline" size="sm">{sortBy === "name" ? "Name" : sortBy === "editedAt" ? "Edited" : "Archived"} {sortDir === "asc" ? "↑" : "↓"}</Button>
                </Menu.Target>
                <Menu.Dropdown>
                  ...
                  <Menu.Item onClick={() => { if (sortBy === "editedAt") setSortDir((d) => d === "asc" ? "desc" : "asc"); else { setSortBy("editedAt"); setSortDir("asc"); } }}>Edited {sortBy === "editedAt" ? (sortDir === "asc" ? " ↑" : " ↓") : null}</Menu.Item>
                  <Menu.Item onClick={() => { if (sortBy === "archivedAt") setSortDir((d) => d === "asc" ? "desc" : "asc"); else { setSortBy("archivedAt"); setSortDir("asc"); } }}>Archived {sortBy === "archivedAt" ? (sortDir === "asc" ? " ↑" : " ↓") : null}</Menu.Item>
                </Menu.Dropdown>
              </Menu>
            </div>
          </div>
          ...
          <ManageJournalTagsModal modalOpened={tagsModalOpened} closeModal={closeTagsModal} />
          <NewJournalModal modalOpened={newJournalModalOpened} closeModal={closeNewJournal} onCreated={() => getJournalsQuery.refetch()} />
        </Stack>
      )}
    </Fragment>
  );
