...
export default function NewJournalEntryModal({
  ...
  const form = useForm({
    ...
    validate: {
      content: (v: string) => (v && v.trim().length > 0 ? null : "Content is required"),
    },
  });
      ...
      >
        <form
          onSubmit={form.onSubmit(async (values) => {
            ...
            const title = values.title && String(values.title).trim().length > 0 ? String(values.title).trim() : undefined;
            const payload: Record<string, unknown> = { content: values.content };
            ...
            if (created) {
              try {
                ...
                const createdObj = created as Record<string, unknown> | undefined;
                const idVal = createdObj ? createdObj["id"] : undefined;
                const createdId = typeof idVal === "number" ? (idVal as number) : undefined;
                if (createdId && tagIds && tagIds.length > 0) {
                  const attachPromises = tagIds.map((tid) => addTagToJournalEntryAsync(createdId, tid));
                ...
                try {
                  await queryClient.fetchQuery({ queryKey: ["journal-entries", journalId], queryFn: () => getJournalEntriesAsync(journalId) });
                } catch (e) {
                  // ignore
                }
              } catch (err) {
