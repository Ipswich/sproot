...
The group evaluates to true *only if both conditions are true*.
...
The group evaluates to true *if either condition is true*.
...
The group evaluates to true *only if one - and only one - condition is true*.
